"""Generate the branded Ecofintec CRM system-documentation PDF.

Rewritten in Phase 10 for the UAE accounting-firm CRM (the system was refactored
from a real-estate platform). Run with the project venv:

    .venv\\Scripts\\python.exe generate_docs.py

Produces Ecofintec_CRM_Documentation.pdf in the repo root. The detailed reference lives
...
doc = SimpleDocTemplate(
    "Ecofintec_CRM_Documentation.pdf",
    pagesize=A4,
    rightMargin=2*cm, leftMargin=2*cm,
    topMargin=2*cm, bottomMargin=2*cm,
    title="Ecofintec CRM — Accounting Firm System Documentation",
    author="Ecofintec CRM",
)
...
story += [
    space(60),
    Paragraph("ECOFINTEC CRM", ParagraphStyle('Cover', fontSize=36, fontName='Helvetica-Bold', textColor=NAVY, alignment=TA_CENTER)),
    space(8),
    Paragraph("Accounting Firm CRM · UAE", ParagraphStyle('CoverSub', fontSize=14, fontName='Helvetica', textColor=GOLD, alignment=TA_CENTER)),
    space(24),
...
story += [
    ...
    space(10),
    body("Ecofintec CRM is a single-firm client-relationship and practice-management tool for a UAE accounting "
         "firm. It manages the full client lifecycle — lead intake, client onboarding, engagements "
         "(bookkeeping, VAT, corporate tax, payroll, audit, CFO and more), tasks, documents, UAE VAT "
...
story += [
    ...
    h2("Backend"),
    info_table([
        ("Framework", "FastAPI (Python) — title 'Ecofintec CRM API', version 2.0.0"),
...
story += [
    ...
    feature_table(
        ...
        [
            ...
            ["senior_accountant", "Own + team's clients; full billing/documents; can assign and delete tasks"],
            ["accountant", "Day-to-day work on own assigned clients"],
            ["auditor", "Firm-wide read-only; may update tasks assigned to them"],
            ["payroll_specialist", "Like accountant, scoped to payroll services only"],
            ["tax_consultant", "Like accountant, scoped to VAT / corporate tax / tax consultation services"],
            ["hr_admin", "HR + e-cards only; no accounting access"],
        ],
        [4.5*cm, 12*cm]
    ),
    space(8),
    body("Row scoping (scope_query): admin & auditor see all clients; senior sees own + team; payroll/tax see "
...
story += [
    ...
    space(10),
    body("The client record is the hub of the accounting domain. Beyond contact details it captures the "
    ...
    feature_table(
        ...
        [
            ...
            ["CRM", "status (active/paused/archived), assigned accountant, source lead, notes"],
        ],
        [4.5*cm, 12*cm]
    ),
    space(8),
...
story += [
    ...
    feature_table(
        ...
        [
            ["Bookkeeping", "Monthly", "1,500"],
            ["VAT Filing", "Quarterly", "1,000"],
            ["Corporate Tax", "Annual", "3,500"],
            ["Payroll", "Monthly", "800"],
            ["Audit", "Annual", "8,000"],
            ["CFO Services", "Monthly", "5,000"],
            ["Financial Statements", "Annual", "4,000"],
            ["Company Formation", "One-time", "12,000"],
            ["Tax Consultation", "One-time", "2,000"],
        ],
...
story += [
    ...
    feature_table(
        ["Aspect", "Detail"],
        [
            ["Categories", "trade_license, vat_return, ct_return, financial_statement, bank_statement, invoice, receipt, contract, passport_eid, audit_report, payroll, other"],
...
story += [
    ...
    feature_table(
        ...
        [
            ...
            ["Corporate Tax (9%)", "Registration (via CT number) and return due 9 months after FY-end"],
...
story += [
    ...
    space(20),
    divider(),
    Paragraph("Ecofintec CRM · Accounting Firm System Documentation · Confidential",
              ParagraphStyle('Footer', fontSize=9, fontName='Helvetica', textColor=GRAY, alignment=TA_CENTER)),
]

doc.build(story)
print("PDF generated: Ecofintec_CRM_Documentation.pdf")
