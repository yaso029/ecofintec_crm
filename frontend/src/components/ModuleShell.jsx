...
export default function ModuleShell({ moduleLabel, navItems = [] }) {
  ...
  return (
    <div className="flex min-h-screen bg-page dark:bg-surface-dark">
      <aside className="fixed top-0 start-0 z-[100] flex h-screen w-60 flex-col bg-gradient-to-b from-primary to-primary-dark text-white">
        ...
        <div className="px-6 pt-7 pb-5 border-b border-white/10">
          ...
          <div className="mt-0.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-accent-light">
            {t('Accounting CRM', 'Accounting CRM')}
          </div>
          {moduleLabel && (
      ...
      <div className="ms-60 flex min-h-screen flex-1 min-w-0 flex-col">
        <header className="sticky top-0 z-50 flex h-[60px] items-center gap-3 border-b border-[var(--border)] bg-[var(--surface)] px-7 shadow-soft">
          <span className="text-[13px] text-[var(--text-muted)]">{t(moduleLabel)}</span>
          <span className="ms-auto text-[13px] font-semibold text-[var(--text)]">{user?.full_name}</span>
