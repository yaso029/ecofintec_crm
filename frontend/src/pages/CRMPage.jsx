      ...
      <div className="modal w-[480px] max-h-[85vh] overflow-y-auto p-7">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-x-4">
            ...
            {(user?.role === 'admin' || user?.role === 'senior_accountant') && teamMembers.length > 0 && (
...
function DashboardTab({ leads, dashStats }) {
  ...
  return (
    <div>
      {/* KPI strip */}
      <div style={{ background: 'linear-gradient(135deg,#0A2E1E,#071a10)', borderRadius: 16, padding: '28px 32px', marginBottom: 24, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }}>
        {kpis.map(k => (
        ...
        {recentLeads.length === 0 ? (
          ...
          >
            ...
            {lead.estimated_value && (
              <div style={{ fontSize: 12, fontWeight: 700, color: '#1F7A59', whiteSpace: 'nowrap' }}>{lead.estimated_value}</div>
            )}
            ...
            {col.leads.map(lead => (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  {lead.estimated_value && <span style={{ fontSize: 11, fontWeight: 800, color: '#1F7A59' }}>{lead.estimated_value}</span>}
                </div>
              </div>
            ))}
          </div>
...
function LeadsTab({ leads, teamMembers, onLeadUpdated }) {
  ...
  return (
    <div>
      ...
      {/* Bulk toolbar */}
      {selCount > 0 && (
        <div style={{ marginBottom: 12, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, borderRadius: 10, background: '#0A2E1E', padding: '10px 16px' }}>
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <div key={lead.id} onClick={() => navigate(`/crm/leads/${lead.id}`)}
                ...
                style={{ borderLeft: isSelected ? '3px solid #1F7A59' : '3px solid transparent' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                  ...
                  {lead.estimated_value && <span style={{ fontSize: 12, fontWeight: 600, color: '#1F7A59' }}>{lead.estimated_value}</span>}
    ...
    <div style={{ minHeight: '100vh', background: 'var(--page-bg, #F0F4F2)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: 'var(--surface, #fff)', borderBottom: '1px solid var(--border, #E2E8E4)', padding: '0 28px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, boxShadow: '0 1px 6px rgba(0,0,0,0.04)', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 900, color: 'var(--text, #0A2E1E)' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {can('leads', 'create') && (
            <button onClick={() => setShowAdd(true)} style={{ padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: '#0A2E1E', color: '#fff', border: 'none' }}>+ Add Lead</button>
          )}
        </div>
      </div>
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{
                ...
                color: isActive ? '#0A2E1E' : 'var(--text-muted, #6B7280)',
                borderBottom: isActive ? '2px solid #0A2E1E' : '2px solid transparent',
            ...
            >
              ...
              {tab.label}
              {count !== null && !loading && (
                <span style={{ background: isActive ? '#0A2E1E' : 'var(--surface-2, #E8EDE9)', color: isActive ? '#fff' : 'var(--text-muted)', borderRadius: 20, padding: '1px 8px', fontSize: 11, fontWeight: 700 }}>
                  {count}
                </span>
              )}
            </button>
