...
const ROLE_COLORS = { admin: '#8b5cf6', senior_accountant: '#3b82f6', accountant: '#10b981', auditor: '#f59e0b', payroll_specialist: '#ec4899', tax_consultant: '#06b6d4', hr_admin: '#0d7377' };
const ROLE_LABELS = { admin: 'Admin', senior_accountant: 'Senior Accountant', accountant: 'Accountant', auditor: 'Auditor', payroll_specialist: 'Payroll Specialist', tax_consultant: 'Tax Consultant', hr_admin: 'HR Admin' };
...
function UserModal({ user, teamLeaders, onClose, onSaved }) {
  ...
  const [form, setForm] = useState(user
    ...
    : { full_name: '', password: '', role: 'accountant', team_leader_id: '' }
  );
  const [loading, setLoading] = useState(false);
    ...
    <div className="modal-overlay">
      <div className="modal w-[420px] p-7">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {form.role === 'accountant' && teamLeaders.length > 0 && (
            <>
...
export default function UsersPage() {
  ...
  const teamLeaders = users.filter(u => u.role === 'senior_accountant' && u.is_active);
    ...
    <div>
      ...
      <div className="card overflow-hidden">
        <table className="data-table">
          <thead>
            <tr>
              {['Name', 'Role', 'Senior Accountant', 'Status', 'Actions'].map(h => (
