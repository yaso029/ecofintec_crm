...
const ROLE_LABELS = { admin: 'Administrator', senior_accountant: 'Senior Accountant', accountant: 'Accountant', auditor: 'Auditor', payroll_specialist: 'Payroll Specialist', tax_consultant: 'Tax Consultant', hr_admin: 'HR Admin' };
...
export default function SettingsPage() {
  ...
  const isYaso = user?.email === 'yaso@ecofintec.com';
  ...
  const [addForm, setAddForm] = useState({ full_name: '', email: '', password: '', role: 'accountant', team_leader_id: '' });
  const [addSaving, setAddSaving] = useState(false);
  const [addMsg, setAddMsg] = useState('');

  const teamLeaders = users.filter(u => u.role === 'senior_accountant');
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      setAddForm({ full_name: '', email: '', password: '', role: 'accountant', team_leader_id: '' });
        ...
        <Section title="My Account">
          ...
          <div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              {user?.role === 'accountant' && <span className="text-xs text-[var(--text-muted)]">Access: CRM, Clients, Calendar</span>}
              {user?.role === 'senior_accountant' && <span className="text-xs text-[var(--text-muted)]">Access: CRM (team scope), Clients, Calendar</span>}
              {user?.role === 'auditor' && <span className="text-xs text-[var(--text-muted)]">Access: Read-only across clients, services & tasks</span>}
              {user?.role === 'payroll_specialist' && <span className="text-xs text-[var(--text-muted)]">Access: Payroll services & tasks, Calendar</span>}
              {user?.role === 'tax_consultant' && <span className="text-xs text-[var(--text-muted)]">Access: VAT / Corporate Tax / Tax Consultation services, Calendar</span>}
                ...
                <form onSubmit={handleAddUser} className="mb-6 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {addForm.role === 'accountant' && (
                      <div className="mb-4">
                        <label className="label">Senior Accountant</label>
