        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="input" value={form.trn || ''} onChange={set('trn')} placeholder="100xxxxxxxxxxxx" maxLength={15} />
            </div>
            <div>
              <label className="label">CT Registration #</label>
              <input className="input" value={form.ct_registration_number || ''} onChange={set('ct_registration_number')} />
            </div>
            <div>
              <label className="label">Trade License #</label>
              <input className="input" value={form.trade_license_number || ''} onChange={set('trade_license_number')} />
            </div>
            <div>
              <label className="label">Trade License Emirate</label>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select className="input flex-1" value={form.fiscal_year_end_month || 12} onChange={set('fiscal_year_end_month')}>
                  {Array.from({ length: 12 }, (_, i) => i + 1).map(m => (
                    <option key={m} value={m}>{new Date(2000, m - 1).toLocaleString('default', { month: 'short' })}</option>
                  ))}
                </select>
                <select className="input w-20" value={form.fiscal_year_end_day || 31} onChange={set('fiscal_year_end_day')}>
                  {Array.from({ length: 31 }, (_, i) => i + 1).map(d => <option key={d} value={d}>{d}</option>)}
                </select>
              </div>
            </div>
            <div className="flex items-center gap-2 pt-[22px]">
              <input type="checkbox" id="esr" checked={!!form.esr_applicable} onChange={set('esr_applicable')} className="cursor-pointer" />
              <label htmlFor="esr" className="cursor-pointer text-[13px] text-gray-600">ESR Applicable</label>
            </div>
          </div>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            <div>
              <label className="label">Assigned Accountant</label>
