...
/* ─── Palette ──────────────────────────────────────────────────────────────── */
const C = {
  revenue: '#1F7A59', collected: '#2E9B72', outstanding: '#D97706',
  active: '#10b981', paused: '#f59e0b', archived: '#94a3b8',
};
const STATUS_TASK = { todo: '#64748b', in_progress: '#3b82f6', blocked: '#ef4444', done: '#10b981' };
const SERVICE_COLORS = ['#1F7A59', '#2E9B72', '#3b82f6', '#8b5cf6', '#f59e0b', '#06b6d4', '#ec4899', '#14b8a6', '#f43f5e'];
const COMPLIANCE = {
...
export default function ClientReportsPage() {
  ...
  const exportCSV = () => {
    ...
    push(['Upcoming VAT Filings', k.upcoming_vat_filings]);
    ...
    push([]);
    push(['Team Workload']);
    push(['Accountant', 'Open Tasks', 'Completed', 'Urgent']);
    data.tasks.team_workload.forEach((t) => push([t.accountant, t.open, t.completed, t.urgent]));
    push([]);
  ...
  return (
    <>
      ...
      <div className="mt-6 flex flex-wrap gap-4">
        ...
        <KpiCard icon="📋" label="Upcoming VAT Filings" value={k.upcoming_vat_filings} accent="#3b82f6">
        ...
        <div className={`grid gap-5 ${isMobile ? 'grid-cols-1' : 'grid-cols-2'}`}>
          ...
          <ChartFrame title="Outstanding by Month">
            <BarChart data={data.revenue.monthly} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <XAxis dataKey="label" tick={axisTick} />
              <YAxis tick={axisTick} tickFormatter={compact} />
              <Tooltip {...tooltipStyle} cursor={{ fill: 'rgba(31,122,89,0.06)' }} formatter={(v) => money0(v, cur)} />
      ...
      <Section title="Client Status" subtitle="Distribution across status, service type, and assigned accountant.">
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <ChartFrame title="Clients by Service Type" height={220}>
            <BarChart data={serviceBars} layout="vertical" margin={{ top: 0, right: 12, left: 10, bottom: 0 }}>
              ...
              <Tooltip {...tooltipStyle} cursor={{ fill: 'rgba(31,122,89,0.06)' }} />
          ...
          <Card>
            <h3 className="mb-3 text-[13px] font-semibold text-[var(--text)]">Clients by Accountant</h3>
            <div className="flex flex-col gap-3">
              {data.client_status.by_accountant.length === 0 ? (
                ...
                <div key={a.accountant}>
                  <div className="mb-1 flex items-center justify-between text-[13px]">
                    <span className="text-[var(--text)]">{a.accountant}</span>
        ...
        <div className={`grid gap-5 ${isMobile ? 'grid-cols-1' : 'grid-cols-5'}`}>
          ...
          <Card className={isMobile ? '' : 'col-span-3'}>
            ...
            <DataTable
              ...
              columns={[
                { key: 'accountant', label: 'Accountant', render: (r) => <span className="font-medium text-[var(--text)]">{r.accountant}</span> },
