        ...
        {[{ value: '', label: 'All' }, ...STATUSES].map(s => {
          ...
          return (
            <button key={s.value} onClick={() => setFilter(s.value)}
              className={`rounded-lg border-[1.5px] px-4 py-1.5 text-xs font-bold ${active ? 'border-primary bg-primary text-white dark:border-accent-light dark:bg-accent-light' : 'border-[var(--border)] bg-[var(--surface)] text-[var(--text-muted)]'}`}>
              {s.label}
            </button>
          );
        })}
