...
Ecofintec CRM is a two-tier application: a **FastAPI** JSON API backed by a SQL
...
| Concern | Choice |
|---------|--------|
| Web framework | FastAPI 0.111 (`backend/main.py`, `app = FastAPI(title="Ecofintec CRM API", version="2.0.0")`) |
| ASGI server | uvicorn |
