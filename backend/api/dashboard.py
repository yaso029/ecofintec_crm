...
def admin_dashboard(current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    accountants = db.query(User).filter(User.role == "accountant", User.is_active == True).count()
    senior_accountants = db.query(User).filter(User.role == "senior_accountant", User.is_active == True).count()
    total_leads = db.query(Lead).count()
    closed_won = db.query(Lead).filter(Lead.stage.in_(WON_STAGES)).count()

    accountant_performance = (
        db.query(User.id, User.full_name, func.count(Lead.id).label("lead_count"))
        .outerjoin(Lead, Lead.assigned_to == User.id)
        .filter(User.role == "accountant", User.is_active == True)
    ...
    return {
        ...
        "accountant_performance": [
            ...
            for a in accountant_performance
        ],
