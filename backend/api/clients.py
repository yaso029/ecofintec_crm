"""Clients API — accounting firm CRM (UAE focused).

Single-firm tool: no firm_id scoping. Role-scoped visibility:
- admin: sees all clients
- senior_accountant: sees own + team's assigned clients
- accountant: sees own assigned clients
"""
...
def scope_query(query, current_user: User, db: Session):
    ...
    - admin / auditor: firm-wide visibility
    - senior_accountant: own + team's assigned clients
    - payroll_specialist / tax_consultant: clients that have a service of the
      role's scoped types (see SERVICE_TYPE_SCOPE)
    - accountant (and any other role): own assigned clients
    """
