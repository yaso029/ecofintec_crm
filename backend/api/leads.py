...
from backend.services.auth_service import get_current_user, require_admin, require_admin_or_senior_accountant, require_permission
...
# Accounting-firm pipeline stages
LEAD_STAGES = [
    "inquiry",
    "discovery_call",
    "documents_requested",
...
def get_leads_query(current_user: User, db: Session):
    ...
    if current_user.role == "admin":
        return query
    elif current_user.role == "senior_accountant":
...
def assign_lead(lead_id: int, req: AssignRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    accountant = db.query(User).filter(User.id == req.user_id, User.is_active == True).first()
    if not accountant:
    ...
    activity = Activity(
        ...
        user_id=current_user.id,
        type="note",
        content=f"Lead reassigned from {old_assignee} to {accountant.full_name}",
    )
    db.add(activity)
    ...
    notify_admins(db, f"👤 {lead.full_name} reassigned from {old_assignee} → {accountant.full_name} by {current_user.full_name}", lead_id=lead.id, exclude_id=current_user.id)
    db.commit()
    db.refresh(lead)
    return lead_to_dict(lead)
