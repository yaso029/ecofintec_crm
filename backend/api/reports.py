...
accountant only sees their own book while admins see the whole firm.
"""
...
ACCOUNTING_ROLES = ("admin", "senior_accountant", "accountant", "auditor",
                    "payroll_specialist", "tax_consultant")
...
def _compliance_for_client(c: Client, today: date) -> list[dict]:
    ...
    if has_trn:
        ...
        for pe in period_ends:
            ...
            if deadline >= today - timedelta(days=20):
                out.append(_deadline(c, "VAT Filing", deadline, today))
                break

    # Corporate Tax — return due 9 months after the financial year end.
    if c.fiscal_year_end_month:
        ...
        out.append(_deadline(c, "Corporate Tax", ct, today))
...
def client_reports(
    ...
    comp_summary = {
        "vat_due": sum(1 for d in deadlines if d["type"] == "VAT Filing"),
        "ct_due": sum(1 for d in deadlines if d["type"] == "Corporate Tax"),
    ...
    upcoming_vat = sum(1 for d in deadlines if d["type"] == "VAT Filing"
    ...
    for aid, n in acc_counts.items():
        by_accountant.append({"accountant": accountant_name.get(aid, "Unassigned"), "count": n})
    ...
    team_workload = [{
        "accountant": accountant_name.get(aid, f"User #{aid}"),
