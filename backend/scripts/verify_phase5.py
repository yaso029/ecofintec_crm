...
DEMO_PASS = "Demo@123"
DEMO = [("Demo Accountant", "accountant"), ("Demo Auditor", "auditor")]
...
def main():
    ...
    r = client.post("/api/invoices", headers=auth(acct), json={
        ...
        "line_items": [
            {"description": "Bookkeeping", "quantity": 1, "unit_price": 1000},
            {"description": "VAT filing", "quantity": 2, "unit_price": 500},
        ],
    })
    inv = r.json()
    check("accountant creates invoice", r.status_code in (200, 201))
    ...
    # --- RBAC -----------------------------------------------------------------
    print("\n-- RBAC --")
    r = client.post("/api/invoices", headers=auth(auditor), json={
        "client_id": client_id, "line_items": [{"description": "x", "quantity": 1, "unit_price": 10}]})
    check("auditor blocked from creating invoice (403)", r.status_code == 403)
    r = client.get("/api/invoices", headers=auth(auditor))
    check("auditor can read invoices (200)", r.status_code == 200)
    r = client.delete(f"/api/invoices/{inv_id}", headers=auth(acct))
    check("accountant blocked from deleting invoice (403)", r.status_code == 403)
