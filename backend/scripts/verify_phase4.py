...
def main():
    ...
    if not admin:
        ...
        return
    seed()
    acct = login("Demo Accountant", DEMO_PASS)
    auditor = login("Demo Auditor", DEMO_PASS)
    check("logins ok", all([acct, auditor]))

    # A client to attach documents to (assigned to the accountant so it's visible).
    acct_id = SessionLocal().query(User).filter(User.full_name == "Demo Accountant").first().id
    ...
    check("accountant uploads document", r.status_code in (200, 201))
    ...
    r = client.post("/api/documents", headers=auth(auditor), files={"file": ("x.pdf", PDF_BYTES, "application/pdf")},
                    data={"client_id": client_id, "category": "other"})
    check("auditor blocked from upload (403)", r.status_code == 403)
    ...
    check("accountant lists documents", r.status_code == 200 and any(d["id"] == doc_id for d in r.json()))
    r = client.get(f"/api/documents?client_id={client_id}", headers=auth(auditor))
    check("auditor can read documents (200)", r.status_code == 200)
    ...
    check("accountant blocked from access log (403)", r.status_code == 403)

    # --- delete RBAC + durable audit -----------------------------------------
    print("\n-- delete --")
    r = client.delete(f"/api/documents/{doc_id}", headers=auth(acct))
    check("accountant blocked from delete (403)", r.status_code == 403)
