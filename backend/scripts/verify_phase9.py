...
def seed_accountant():
    db = SessionLocal()
    try:
        u = db.query(User).filter(User.full_name == "Demo Accountant").first()
        if u:
            u.role, u.is_active, u.password_hash = "accountant", True, hash_password(DEMO_PASS)
        else:
            db.add(User(full_name="Demo Accountant", email="accountant@demo.crm",
                        password_hash=hash_password(DEMO_PASS), role="accountant", is_active=True))
        db.commit()
    finally:
        db.close()
...
def main():
    ...
    acct = login("Demo Accountant", DEMO_PASS).json().get("access_token")
    ...
    r = client.post("/api/users", headers=auth(admin), json={"full_name": "Audit Probe", "password": "probe123", "role": "accountant"})
    ...
    for _ in range(3):
        login("Demo Accountant", "wrongpw")  # 3 failures (< threshold)
    ok = login("Demo Accountant", DEMO_PASS)
    ...
    check("counter reset after success", login("Demo Accountant", "wrongpw").status_code == 401)
