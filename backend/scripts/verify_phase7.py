...
def seed_auditor():
    db = SessionLocal()
    try:
        u = db.query(User).filter(User.full_name == "Demo Auditor").first()
        if u:
            u.role, u.is_active, u.password_hash = "auditor", True, hash_password(DEMO_PASS)
        else:
            db.add(User(full_name="Demo Auditor", email="auditor@demo.crm",
                        password_hash=hash_password(DEMO_PASS), role="auditor", is_active=True))
        db.commit()
    finally:
        db.close()
...
def main():
    ...
    # auditor (read-only) can view checklist
    r = client.get(f"/api/clients/{cid}/tax-checklist", headers=auth(auditor))
    check("auditor can read checklist", r.status_code == 200)
