...
SECRET_KEY = os.environ.get("SECRET_KEY", "ecofintec-crm-secret-key-change-in-production")
ALGORITHM = "HS256"
...
def require_admin_or_senior_accountant(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in ("admin", "senior_accountant"):
        raise HTTPException(status_code=403, detail="Admin or Senior Accountant access required")
    return current_user
...
def require_payroll_specialist(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in ("admin", "payroll_specialist"):
        raise HTTPException(status_code=403, detail="Payroll Specialist access required")
    return current_user


def require_tax_consultant(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in ("admin", "tax_consultant"):
        raise HTTPException(status_code=403, detail="Tax Consultant access required")
    return current_user
