...
# Accounting domain (Phase 2): Clients, Services, Tasks
...
def seed_admin():
    db = SessionLocal()
    try:
        NEW_EMAIL = "yaso@ecofintec.com"
        # Migrate any existing admin (including pre-rebrand Ecofintec emails) by the
...
@app.get("/")
def root():
    return {"status": "Ecofintec CRM API running", "version": "2.0.0"}
